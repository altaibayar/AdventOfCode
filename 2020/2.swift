...
//  AdventOfCodeTest
//
//  Created by altaibayar tseveenbayar on 2020-12-02.
//
...
extension Password {
    static func parse(_ str: String) -> Password {
        let arr = str.split(separator: " ");

        let mi = Int(arr[0].split(separator: "-")[0])!
        let ma = Int(arr[0].split(separator: "-")[1])!
        let ch = String(arr[1].split(separator: ":")[0]);
        let pa = String(arr[2])

        return Password(min: mi, max: ma, char: ch, content: pa)
    }

    var isValid: Bool {
        let count = self.content.filter { String($0) == self.char }.count;
        return self.min <= count && count <= self.max
    }

    var isValid2: Bool {
        let result1 = String(self.content[self.content.index(self.content.startIndex, offsetBy: self.min - 1)]) == self.char
        let result2 = String(self.content[self.content.index(self.content.startIndex, offsetBy: self.max - 1)]) == self.char
