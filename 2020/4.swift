...
extension Passport {
    static func parse(lines: [String]) -> Passport {
        ...
        for line in lines {
            let pairs = line.split(separator: " ").map { String($0) }
            for pair in pairs {
                let kvp = pair.split(separator: ":").map { String($0) }
                fields[kvp[0]] = kvp[1]
            }
        }
        ...
        return Passport(
            ...
            byr: fields["byr"] == nil ? nil : Int(fields["byr"]!!),
            iyr: fields["iyr"] == nil ? nil : Int(fields["iyr"]!!),
            eyr: fields["eyr"] == nil ? nil : Int(fields["eyr"]!!),
            hgt: fields["hgt"] ?? nil,
            hcl: fields["hcl"] ?? nil,
            ecl: fields["ecl"] ?? nil,
            pid: fields["pid"] ?? nil
...
extension Passport {
    var hasValidByr: Bool {
        guard let byr = self.byr else { return false }
        return 1920 <= byr && byr <= 2002
    }
    
    var hasValidIyr: Bool {
        guard let iyr = self.iyr else { return false }
        return 2010 <= iyr && iyr <= 2020
    }
    
    var hasValidEyr: Bool {
        guard let eyr = self.eyr else { return false }
        return 2020 <= eyr && eyr <= 2030
    }
    
    var hasValidHeight: Bool {
        guard let hgt = self.hgt else { return false }
        
        let intPart: (String) -> Int? = { str in
            let valStr = str[str.startIndex ..< str.index(str.startIndex, offsetBy: str.count - 2)]
            return Int(valStr)
        }
        
        if hgt.hasSuffix("cm") {
            if let val = intPart(hgt), 150 <= val && val <= 193 { return true }
        } else if hgt.hasSuffix("in") {
            if let val = intPart(hgt), 59 <= val && val <= 76 { return true }
        }
        
        return false
    }
    
    var hasValidHairColor: Bool {
        guard let hcl = self.hcl else { return false }
        guard hcl.hasPrefix("#") && hcl.count == 7 else { return false }
        
        let clr = String(hcl[hcl.index(hcl.startIndex, offsetBy: 1) ..< hcl.endIndex])
        return clr.allSatisfy { $0.isHexDigit }
    }
    
    var hasValidEcl: Bool {
        guard let ecl = self.ecl else { return false }
        let vals: Set<String> = Set<String>(arrayLiteral: "amb", "blu", "brn", "gry", "grn", "hzl", "oth")
        return vals.contains(ecl)
    }
    
    var hasValidPid: Bool {
        guard let pid = self.pid, pid.count == 9 else { return false }
        return pid.allSatisfy{ $0.isNumber }
    }
    
    var hasRequiredFields: Bool {
        let reqFields = Set<String>([
            "byr",
            "iyr",
            "eyr",
            "hgt",
        ...
        return self.fields.isSuperset(of: reqFields);
    }
    
    var hasValidValues: Bool {
        return self.hasValidByr &&
            self.hasValidIyr &&
            self.hasValidEyr &&
            self.hasValidHeight &&
            self.hasValidHairColor &&
            self.hasValidEcl &&
            self.hasValidPid
    }
}

func parsePassports(passport_lines: [String]) -> [Passport] {
    var result = [Passport]();
    
    var acc: [String] = []
    var i = 0
    while i < passport_lines.count {
        let line = passport_lines[i]
        if line.isEmpty {
            result.append(Passport.parse(lines: acc))
            acc.removeAll()
        } else {
            acc.append(line)
        }
...
let passports = parsePassports(passport_lines: passports_long)
print(passports.filter { $0.hasRequiredFields }.count)
print(passports.filter { $0.hasRequiredFields && $0.hasValidValues }.count)
