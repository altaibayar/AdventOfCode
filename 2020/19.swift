...
//  Advent19
//
//  Created by altaibayar tseveenbayar on 2020-12-19.
//
...
extension Rule {
    static func parse(line: String) -> Rule {
        let splt = line.split(separator: ":").map { String($0) }
        let id: Int = Int(splt[0])!

        let potentialLiteral = splt[1].trimmingCharacters(in: CharacterSet(charactersIn: " \""))
        if potentialLiteral == "a" || potentialLiteral == "b" {
            return Rule(id: id, matches: .literal(potentialLiteral))
        }

        let ruleParts = splt[1]
            .split(separator: "|")
            .map { String($0) }

        let matches: [[Int]] = ruleParts.map { part in
            return part.split(separator: " ").filter { !$0.isEmpty }.map { Int($0)! }
        }

        return Rule(id: id, matches: .rule(matches))
    }
}

func parse(input: [String]) -> (rules: [Rule], messages: [String]) {
    let idx = input.firstIndex(of: "")!

    let rules = input.prefix(upTo: idx).map { Rule.parse(line: String($0)) }
    let messages: [String] = input.suffix(from: idx + 1).map { String($0) }

    return (rules: rules, messages: messages)
}

// cheat for part2
var deepCheckIds = Set<Int>()
var deepness: [Int: Int] = [:]
let MAX_DEEP = 2

func literalify(from rootRule: Rule, current: String, rules: [Rule]) -> [String] {
    if case let .literal(ch) = rootRule.matches {
        return ["\(current)\(ch)"]
    }

    guard case let .rule(ruleIds) = rootRule.matches else {
        fatalError()
    }
    ...
    for ids in ruleIds {
        ...
        for ruleId in ids {
            ...
            if acc.isEmpty {
                acc.append(contentsOf: literals)
            } else {
                acc = literals.flatMap { l in acc.map { $0 + l } }
            }
        }

        acc.forEach { result.append(current + $0) }
    }

    return result
}

func buildAll(from rootId: Int, rules: [Rule]) -> Set<String> {
    let rootRule = rules.first(where: { $0.id == rootId })!
    return Set<String>(literalify(from: rootRule, current: "", rules: rules))
}

func part1(input: [String]) {
    let (rules, messages) = parse(input: input)

    let all = buildAll(from: 0, rules: rules)
    print("All: \(all.count)")

    let passingCount = messages.filter { all.contains($0) }.count
    print("Result: \(passingCount)")
}

func part2(input: [String]) {
    var input = input

    let replaces: [String: String] = [
        "8: 42": "8: 42 | 42 8",
        "11: 42 31": "11: 42 31 | 42 11 31",
    ]

    for (key, val) in replaces {
        if let idx = input.firstIndex(of: key) {
            input[idx] = val
        } else {
            fatalError()
        }
    ...
    deepness.removeAll()
    deepCheckIds.insert(8)
    deepCheckIds.insert(11)
