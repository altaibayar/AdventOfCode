...
extension Group {
    static func parse(lines: [String]) -> [Group] {
        var result = [Group]()
        ...
        while i < lines.count {
            ...
            if cur.isEmpty {
                result.append(Group(answers: acc))
                acc.removeAll()
                accReseted = true
            } else {
                // part 1
                // acc.formUnion(cur.map { String($0) })
                
                // part2
                if acc.isEmpty && accReseted {
                    acc.formUnion(cur.map { String($0) })
                    accReseted = false
                } else {
                    acc.formIntersection(cur.map { String($0) })
                }
            }
...
let groups = Group.parse(lines: answers_long)
print(groups.map { $0.answers.count }.reduce(0, +))
