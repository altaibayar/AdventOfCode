...
//  Advent21
//
//  Created by altaibayar tseveenbayar on 2020-12-21.
//
...
func parse(line:  String) -> (ingrediences: [String], alergens: [String]) {
    let splts = line.split(separator: " ").map { String($0) }
    let idx = splts.firstIndex(of: "(contains")!

    let ingrediences: [String] =  splts[0 ..< idx].map { String($0) }
    let alergens = splts[idx + 1 ..< splts.count]
        .map { String($0) }
        .map { $0.trimmingCharacters(in: CharacterSet(charactersIn: ",)")) }

    return (ingrediences: ingrediences, alergens: alergens)
}

func parse(list: [String]) -> (map: [String: [Set<String>]], count: [String: Int]) {
    var map = [String: [Set<String>]]()
    var count = [String: Int]()

    for line in list  {
        let (ingrediences, alergens) = parse(line: line)

        let set = Set<String>(ingrediences)
        assert(set.count == ingrediences.count)

        for alergen in alergens {
            if let _ = map[alergen] {
                map[alergen]?.append(set)
            } else {
                map[alergen] = [set]
            }
        }

        for ing in ingrediences {
            count[ing] = count[ing] == nil ? 1 : count[ing]! + 1
        }
    }
...
func dowork(list: [String]) {
    var (foods, counts) = parse(list: list)
    var alergen_food = [String: String]()
    var food_alergen = [String: String]()

    var foundAlergen: String? = ""
    while foundAlergen != nil {
        foundAlergen = nil

        for (alergen, val) in foods {
            if val.count > 1 {
                var inter = val[0]
                val.forEach { s in inter.formIntersection(s) }

                if inter.count == 1 {
                    // alergen found
                    let food = inter.first!
                    alergen_food[alergen] = food
                    food_alergen[food] = alergen

                    foundAlergen = alergen
                    break
                }

            } else if val.count == 1 && val[0].count == 1 {
                let food = val[0].first!
                alergen_food[alergen] = food
                food_alergen[food] = alergen
        ...
        if let foundAlergen = foundAlergen {
            // remove from counts
            let food = alergen_food[foundAlergen]!

            counts[food] = nil
            // remove alergen
            for (key, _) in foods {
                foods[key] = foods[key]?.compactMap {
                    var copy = $0
                    copy.remove(food)
            ...
            foods.removeValue(forKey: food)
        }
    }

    print("Part1: \(counts.values.reduce(0, +))")

    var ingredients = [String]()
    for alergen in food_alergen.values.sorted() {
        let food = alergen_food[alergen]!
