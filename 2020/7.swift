...
//  AdventCode7
//
//  Created by altaibayar on 07/12/2020.
...
let RULES = rules_long

func parseContainerBag(line: String) -> String {
    var result = [String]()
    
    let splt = line.split(separator: " ").map { String($0) }
    
    var i = 0
    while i < splt.count && splt[i] != "bags" {
...
func parseChildBags(line: String) -> [(count: Int, color: String)] {
    var i = 0
    let splt = line.split(separator: " ").map { String($0) }
    while i < splt.count && splt[i] != "contain" { i += 1}
    
    let rest = splt[i + 1 ..< splt.endIndex].map { String($0) }
    
    i = 0
    guard rest[i] != "no" else { return [] }
    
    var result: [(count: Int, color: String)] = []
    ...
    while i < rest.count {
        let current = rest[i]
        if ["bags.", "bag.", "bags,", "bag,"].contains(current) {
            result.append((count: count, color: color.joined(separator: " ")))
            color.removeAll()
            count = 0
        } else if let c = Int(current) {
            count = c
        } else {
...
func getContainers(all: [String], bag: String) -> [String] {
    let result = all
        .filter { $0.contains(bag) && !$0.hasPrefix(bag) }
        .map { parseContainerBag(line: $0) }
    return result;
}
...
func part1() {
    ...
    while !list.isEmpty {
        ...
        let containers = getContainers(all: RULES, bag: current)
        if containers.isEmpty { continue }
        
        result.formUnion(containers)
        list.append(contentsOf: containers)
    }
...
struct Bag {
    let name: String
    let children: [(count: Int, color: String)];
}

func childrenCount(all: [String: Bag], bagName: String) -> Int {
    let bag = all[bagName]!
    guard !bag.children.isEmpty else { return 0 }
    
    var childCount = 0
    var acc = 0
    for child in bag.children {
        acc += child.count
        childCount += (child.count * childrenCount(all: all, bagName: child.color))
    }

    return acc + childCount
}

func part2(rules: [String], bagName: String = MY_BAG) {
    let bags: [Bag] = rules.map { rule in
        return Bag(name: parseContainerBag(line: rule), children: parseChildBags(line: rule))
    }
    var all = [String: Bag]()
    bags.forEach { b in all[b.name] = b }
    
    print(childrenCount(all: all, bagName: bagName))
}

//part1()
part2(rules: rules_short2)
part2(rules: rules_short, bagName: "shiny gold")
part2(rules: rules_long)
