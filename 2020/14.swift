...
//  Advent14
//
//  Created by altaibayar tseveenbayar on 2020-12-15.
//
...
func parse(list: [String]) -> [Instruction] {
    let parseMask: (String) -> Instruction = { line in
        return .mask(String(line.split(separator: " ").last!))
    }

    let parseMem: (String) -> Instruction = { line in
        let idx1 = line.index(after: line.firstIndex(of: "[")!)
        let idx2 = line.lastIndex(of: "]")!
        let index = String(line[idx1 ..< idx2])

        let value = String(line.split(separator: " ").last!)

        return .mem(Int(index)!, Int(value)!)
    }

    return list.map {
        return $0.hasPrefix("mask") ? parseMask($0) : parseMem($0)
    }
}

func maskValue(_ value: Int, mask: String) -> Int64 {
    var binaryValue = String(value, radix: 2, uppercase: true)
    while binaryValue.count < N { binaryValue = "0" + binaryValue }

    var result = ""
    for i in stride(from: N - 1, through: 0, by: -1) {
        let maskChar = mask.char(at: i)
        let valChar = binaryValue.char(at: i)

        if maskChar == "0" || maskChar == "1" {
            result = maskChar + result
        } else {
            result = valChar + result
        }
    }

    return Int64(result, radix: 2)!
}

func maskValue2(_ value: Int, mask: String) -> String {
    var binaryValue = String(value, radix: 2, uppercase: true)
    while binaryValue.count < N { binaryValue = "0" + binaryValue }
...
func allAddresses(address: String) -> [String] {
    let pair: (String) -> [String] = { str in
        let idx = str.firstIndex(of: "X")!.utf16Offset(in: str)
    ...
    let n = address.components(separatedBy: "X").count - 1

    var result: [String] = [address]
    for _ in 0 ..< n {
        result = result.compactMap { pair($0) }.flatMap { $0 }
    }
...
func part1(list: [String]) {
    ...
    let instructions = parse(list: list)
    var lastMask = ""
    ...
    for instruction in instructions {
        ...
        case .mask(let mask):
            lastMask = mask
        case .mem(let index, let value):
            dict[index] = maskValue(value, mask: lastMask)
        }
    }
...
func part2(list: [String]) {
    ...
    for instruction in instructions {
        ...
        case .mask(let mask):
            lastMask = mask
        case .mem(let index, let value):
            let newMask =  maskValue2(index, mask: lastMask)
            let newMasks = allAddresses(address: newMask)

            newMasks.forEach { i in
                dict[i] = value
            }
        }
    }
