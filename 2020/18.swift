...
//  Advent18
//
//  Created by altaibayar tseveenbayar on 2020-12-18.
//
...
enum Op { case mul, plus }

func parseNumber(line: String, from index: inout Int) -> Int {
    var number = ""
    while index < line.count {
        let ch = line.char(at: index)
...
func exec(op: Op, a: Int, b: Int) -> Int {
    return op == .mul ? a * b : a + b
}

func evaluate(line: String, from index: inout Int, accumulateMul: Bool = false) -> Int {
    var result = 0
    var lastOp: Op?
    ...
    while index < line.count {
        ...
        case ")":
            index += 1
            return result * toMultiply.reduce(1, *)
        case "(":
            index += 1
            let number = evaluate(line: line, from: &index, accumulateMul: accumulateMul)
            if lastOp != nil {
                result = exec(op: lastOp!, a: result, b: number)
            } else {
                result = number
            }
        ...
        default:
            if ch != " " {
                let number = parseNumber(line: line, from: &index)
                if lastOp != nil {
                    result = exec(op: lastOp!, a: result, b: number)
                } else {
                    result = number
...
func part1(input: [String]) {
    let inputs: [Int] = input.map { line in
        var index = 0
        return evaluate(line: line, from: &index)
    }
...
func part2(input: [String]) {
    let inputs: [Int] = input.map { line in
        var index = 0
        return evaluate(line: line, from: &index, accumulateMul: true)
    }
