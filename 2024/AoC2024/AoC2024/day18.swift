...
class Day18 {
    ...
    class Pos: Hashable, CustomDebugStringConvertible {
        ...
        static func == (lhs: Day18.Pos, rhs: Day18.Pos) -> Bool {
    ...
    func parse(input: [String]) -> [(Int, Int)] {
        return input.compactMap {
            let splt = $0.split(separator: ",")
            guard let i1 = Int(splt[0]), let i2 = Int(splt[1]) else { return nil }
            
            return (i1, i2)
        }
    }
    
    func createMap(input: [(Int, Int)], N: Int) -> [[Pos?]] {
        let input = input[..<(N == 7 ? 12 : 1024)]
    ...
    func dowork() {
        ...
        let N = input1.count == input.count ? 7 : 71
        let parsed = parse(input: input)
        
        let map = createMap(input: parsed, N: N)
        
        
//        map.forEach {
//            print($0.map { $0 == nil ? "#" : "." }.joined())
//        }
//        print("---")
        ...
        heap.insert(item: map[0][0]!)
        
        while heap.count > 0 {
            let current = heap.popTop()
//            print("count: \(heap.count): \(current.debugDescription)")
            ...
            [
                ...
                if n.dist == Int.max || n.dist > current.dist + 1 {
                    n.prev = current
                    n.dist = current.dist + 1
                    
                    heap.insert(item: n)
                    
//                    let idx = heap.heap.firstIndex(of: n)!
//                    heap.bubbleUp(idx: idx)
                }
            }
...
//            heap.heapify(0)
        ...
        let targetPos: Pos = map[N - 1][N - 1]!
        print(targetPos.dist)
    }
