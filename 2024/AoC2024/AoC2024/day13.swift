...
class Day13 {
    ...
    static func parseIntegers(from string: String) -> Shift {
        // Create a regular expression pattern to match two integers
        let pattern = #"(\d+)\D+(\d+)"#
        ...
        do {
            ...
            if let match = regex.firstMatch(in: string, range: NSRange(string.startIndex..., in: string)) {
                ...
                // Convert matched ranges to integers
                if let firstRange = Range(firstNumberRange, in: string),
                   let secondRange = Range(secondNumberRange, in: string) {
                    let firstNumber = Int(string[firstRange])!
                    let secondNumber = Int(string[secondRange])!
        ...
        // Return nil if no integers found
        fatalError()
    }
    ...
    struct Joystick {
        ...
        init(lines: [String]) {
            buttonA = Day13.parseIntegers(from: lines[0])
            buttonB = Day13.parseIntegers(from: lines[1])
            prize = Day13.parseIntegers(from: lines[2])
        }
        
//        var isALarger: Bool {
//            let a = max(prize.x / dA.x, prize.y / dA.y)
//            let b = max(prize.x / dB.x, prize.y / dB.y)
//            
//            return a > b * 3
//        }
//        var longer: Shift { isALarger ? dA : dB }
//        var shorter: Shift { isALarger ? dB : dA }
    }
    ...
    func dowork() {
        ...
        while i < input.count {
            joysticks.append(Joystick(lines: input[i ..< i + 3].map { String($0) }))
            i += 4
        }

        var result = 0
        
        for (i, joy) in joysticks.enumerated() {
            print("Processing: (\(i)/\(joysticks.count))")            
            
            let maxPressA: Int = Int(max(
                ceil(Double(joy.prize.x) / Double(joy.buttonA.x)),
                ceil(Double(joy.prize.y) / Double(joy.buttonA.y))
            ))
            let maxPressB: Int = Int(max(
                ceil(Double(joy.prize.x) / Double(joy.buttonB.x)),
                ceil(Double(joy.prize.y) / Double(joy.buttonB.y))
            ))
            
            var minPrice = Int.max
            
            for aCount in 0 ..< maxPressA {
                for bCount in 0 ..< maxPressB {
                    let expectedX = aCount * joy.buttonA.x + bCount * joy.buttonB.x
                    let expectedY = aCount * joy.buttonA.y + bCount * joy.buttonB.y
                    
                    if expectedX == joy.prize.x && expectedY == joy.prize.y {
                        if (3 * aCount + bCount) < minPrice {
                            minPrice = 3 * aCount + bCount
                        }
                    }
                }
            }
...
//            var longerSteps = Int(
//                max(
//                    floor(Double(joy.prize.x) / Double(joy.longer.x)),
//                    floor(Double(joy.prize.y) / Double(joy.longer.y))
//                )
//            )
//            while longerSteps >= 0 {
//                let prizeX = joy.prize.x - (longerSteps * joy.longer.x)
//                let prizeY = joy.prize.y - (longerSteps * joy.longer.y)
//                
//                if prizeX % joy.shorter.x == 0
//                    && prizeY % joy.shorter.y == 0
//                    && prizeX % joy.shorter.x == prizeY % joy.shorter.y
//                {
////                    print("-----")
////                    print(joy)
//                    
//                    let shortSteps = prizeX / joy.shorter.x
//                    
//                    if joy.isALarger {
//                        result += (3 * longerSteps) + shortSteps
//                    } else {
//                        result += (3 * shortSteps) + longerSteps
//                    }
//                }
//                
//                longerSteps -= 1
