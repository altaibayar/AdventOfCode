...
extension Task3 {
    ...
    static func main() {
        let input = Task3.input2
        let regex = /\d+/
        
        var result1 = 0
        var result2 = 0
        
        var matchings = [[StrPart]]()
        
        for (i, l) in input.enumerated() {
            let matches = l.matches(of: regex)
            matchings.append([])
            
            for match in matches {
                var hasGear = false
                
                for j in match.startIndex.utf16Offset(in: l) ..< match.endIndex.utf16Offset(in: l) {
                    for ii in [i - 1, i, i + 1] {
                        for jj in [j - 1, j, j + 1] {
                            
                            if ii >= 0 && ii < input.count - 1 && jj >= 0 && jj < l.count - 1 {
                                let ll = input[ii]
                                if ll[ll.index(ll.startIndex, offsetBy: jj)].isGear {
                                    hasGear = true
                                }
                            }
                ...
                if (hasGear) {
                    matchings[i].append(
                        StrPart(
                            startIndex: match.startIndex.utf16Offset(in: l),
                            endIndex: match.endIndex.utf16Offset(in: l),
                            part: Int(match.output)!)
                    )
                    result1 += Int(match.output)!
                }
            }
        }
        ...
        for (i, l) in input.enumerated() {
            for (j, ch) in l.enumerated() {
                if ch == "*" {
                    var matchingStrs = [StrPart]()
                    
                    if i > 0 {
                        matchingStrs.append(contentsOf: matchings[i - 1].filter { $0.neighbours(i: j) })
                    }
                    
                    matchingStrs.append(contentsOf: matchings[i].filter { $0.neighbours(i: j) })
                    
                    if i < input.count - 1 {
                        matchingStrs.append(contentsOf: matchings[i + 1].filter { $0.neighbours(i: j) })
                    }
                    
                    if matchingStrs.count == 2 {
                        result2 += (matchingStrs.first!.part * matchingStrs.last!.part)
                    }
                }
            }
        }
...
extension StrPart {
    func neighbours(i: Int) -> Bool {
        return i >= startIndex  - 1 && i <= endIndex
    }
}
